...
from sodner.models.MultiHeadAttention import MultiHeadAttention
...
class SodNER(Model):
    ...
    def forward(self,
        ...
        self.attention = MultiHeadAttention(contextualized_embeddings.shape[2], 4,contextualized_embeddings.shape[2]).to("cuda:0")
        attention = self.attention(contextualized_embeddings, text_mask).to("cuda:0")
        conv_output = attention.permute(0, 2, 1)
        contextualized_embeddings = contextualized_embeddings + conv_output
